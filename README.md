...
目前穩定功能以 YouTube 音訊取得、全局調性分析、升降 Key 與 MP3 輸出為主；CPU-only 主旋律簡譜僅作為 preview，不定位為正式樂譜或準確扒譜結果。Host GPU 模式可提供 Demucs stems 與 RMVPE frame-level vocal pitch，但仍屬私人練唱輔助用途。
...
目前採「模組化單體」處理下載、key analysis、beat tracking、CPU-only melody preview、transpose 與 artifact 管理。主 FastAPI 環境保持輕量；Demucs 與 RMVPE 只在 WSL host-native GPU 模式下，透過獨立 GPU venv 的 subprocess 執行。前端仍只連 FastAPI，模型輸出由 FastAPI 正規化並套用既有認證與 job ownership。
...
- Docker Desktop 或 Docker Engine（僅 Docker 部署方式需要）

## 本機開發

這個模式讓前端、FastAPI 與 CPU-only 音訊工具在同一台開發機執行，適合日常開發與功能驗證。Mac 可用 Homebrew；WSL 可用 apt/conda 等既有方式安裝 FFmpeg 與 Rubber Band。
...
在 CPU-only 模式下，主旋律功能直接分析完整混音，沒有做人聲分離，因此可能包含伴奏、和聲或錯誤音符。若在 WSL host-native GPU 模式啟用 Demucs 與 RMVPE，vocals source 可由人聲 stem 產生正式 vocal pitch。
...
Windows Tailscale Serve
        │ localhost:8001
        ▼
WSL FastAPI 或 Docker Compose / FastAPI
```
...
Docker 部署僅支援 CPU-only 功能與 Phase 2A melody preview：
...
若要使用 Demucs / RMVPE host GPU 功能，請不要在 Docker 容器內執行 GPU runtime；改在 WSL host 啟動主 FastAPI，並用 `DEMUCS_PYTHON` / `RMVPE_PYTHON` 指向獨立 GPU venv，詳見 [`docs/gpu.md`](docs/gpu.md)。
...
正式 vocal pitch 由 melody pipeline 在選用 vocals source 時觸發；成功時輸出 `analysis/pitch/vocal_pitch.json`。Phase 2Cb 採 RMVPE-only：若 RMVPE、ONNX Runtime、CUDAExecutionProvider 或 vocals stem 不可用，不會回退成 pYIN，也不會產生假的 `vocal_pitch.json`。
...
- `STEM_SEPARATION_ENABLED`：是否啟用 host stem separation；Docker 部署應維持 `false`
...
- CPU-only melody preview：beat tracking、pYIN、Melody JSON 與 MIDI
...
- pYIN 保留為 preview；正式 vocal pitch 採 RMVPE-only，不做 pYIN fallback
...
- CPU-only pYIN preview 可能受完整混音、多聲部、spoken word 或強烈伴奏干擾。
- Phase 2A 不做自動拍號辨識；`auto` 不確定時回退為不分小節。
- Demucs 可能無法完全分離主唱與和聲，也可能產生音訊 artifact。
- Phase 2Cb vocal pitch backend 採 RMVPE-only；RMVPE、ONNX Runtime、CUDA 或 vocals stem 不可用時，不 fallback 成 pYIN，也不產生假的 `analysis/pitch/vocal_pitch.json`。
- Host GPU 模式目前不支援 Docker；Docker 後端應停用 stem separation。
...
- `requirements.txt`：production FastAPI runtime 需要的套件，Docker image 只安裝這個檔案。
