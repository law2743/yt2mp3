...
- Host GPU 模式下的 Demucs stems、RMVPE cache、四 backend pitch CSV、adaptive melody fusion、Melody JSON、MIDI 與新版簡譜草稿 artifacts
...
- `analysis/rhythm/notes_draft.json`：由既有 pitch timeline、beat grid、vocal onsets 組成的草稿音符 JSON
...
前端在 Step 2 區塊會優先顯示 `jianpu_draft.txt`，並提供簡譜 TXT、`numbered_notation.json`、`notes_draft.csv` 下載；如果新版 artifact 無法讀取，仍會回退到既有 melody preview。
