...
日期：2026-06-19  
目標讀者：Codex／開發者  
開發目標：建立一個可部署至 Render 的輕量 Web 服務，讓使用者貼上 YouTube 網址、分析歌曲調性，選擇升降半音後輸出 MP3。
...
- 顯示下載、分析、轉調、完成與失敗狀態。
- 自動清除暫存檔案。
- Docker 化並提供 Render 部署設定與 README。
...
- Phase 2：Render 只負責 UI 與任務管理；地端 Worker 主動領取下載／轉檔任務。
...
- 頁面重新整理後嘗試讀取 job 狀態。
- Render 重啟造成 job 遺失時，顯示「暫存工作已失效，請重新分析」，不得無限重試。
...
- 單一 Render Web Service 較容易部署與除錯。
- 不需處理跨網域、前後端版本同步與兩個免費服務喚醒。
- 前端可使用原生 HTML／CSS／JavaScript，不需要 React。
- 沒有帳號、資料庫與複雜狀態，不值得引入大型框架。
...
### 6.4 Phase 2 地端 Worker 架構

若 Render 的資料中心 IP 被 YouTube 限制，或轉檔效能不足，改為：

```text
Render Web/API + Job Queue
          ▲
          │ HTTPS 主動輪詢／領取任務
          │
地端 Worker（住宅網路）
...
優先採「地端 Worker 主動領工作」，而非讓 Render 主動進入整個 Tailnet。若沿用既有 Tailscale API，仍須使用獨立 API token、最小權限與 timeout。
...
### 11.3 Render ephemeral filesystem

- 不依賴檔案跨重啟存在。
- 重啟後所有記憶體 job 視為失效。
- MVP 不使用 persistent disk。
...
- 建議方案：環境變數 `APP_PASSWORD`，首頁以簡單 session login 保護。
- Cookie 必須為 `HttpOnly`、`Secure`、`SameSite=Lax`。
- Session signing key 由 `SESSION_SECRET` 提供，不可寫死在程式碼。
- API 端點除 `/health` 與登入頁外皆需驗證 session。

不得只靠隱藏網址當作安全措施。

### 12.3 Rate limit

- 每個 session／來源 IP：每分鐘最多 5 次建立工作。
- 同一 session 同時最多 1 個 active job。
...
- 不永久保存輸入 URL、音訊或下載紀錄。
- log 不記錄 session cookie、密碼、token 或 cookies。
- URL 可只記錄 video ID。
...
- production 缺少 `APP_PASSWORD` 或 `SESSION_SECRET` 時啟動失敗。
...
- 建立單一 Docker Web Service。
- Health check path：`/health`。
- 不宣告 persistent disk。
- secrets 標示 `sync: false`。
...
- [ ] `docker build` 成功。
- [ ] `docker run` 後 `/health` 正常。
- [ ] Render 可由 `render.yaml` 部署。
- [ ] Render 重啟後不會因殘留 job 狀態而啟動失敗。
...
- Key 分析：Render CPU 目標 60 秒內。
- 轉調＋MP3：Render CPU 目標 2～4 分鐘內。
...
1. 建立 FastAPI 骨架、設定、登入、首頁與 `/health`。
...
9. 完成前端 polling、狀態、按鈕、下載與手機版。
10. 補齊 rate limit、取消、timeout、路徑安全與 cleanup。
11. 建立 Dockerfile、render.yaml、README、THIRD_PARTY_NOTICES。
...
5. 可部署至 Render 並通過 `/health`。
6. iPhone Safari 可正常操作與下載。
7. 已清楚揭露 YouTube 雲端 IP、調性偵測準確度、Render 暫存及內容授權限制。
...
| 風險 | 影響 | MVP 決策 |
|---|---|---|
| YouTube 限制 Render 資料中心 IP | 無法取得部分影片 | 顯示明確錯誤；Phase 2 改地端 Worker |
| Key 偵測存在相對大小調／轉調誤判 | 顯示調名可能不準 | 顯示可信度與候選；半音轉調仍可正常使用 |
| Render CPU 轉調較慢 | 使用者等待 | 單併發、背景 job、進度提示、限制片長 |
| Render 重啟造成工作消失 | 需重新分析 | MVP 接受 ephemeral 設計並清楚提示 |
| GPL 元件授權 | 影響閉源商用 | MVP 採開源方向；商業化前重新審查 |
| 公開服務被濫用 | 成本、封鎖與法律風險 | 密碼保護、rate limit、單 session active job |
| 完整 WAV 佔用大量磁碟 | Render 空間不足 | 限制片長與來源大小、TTL cleanup、控制併發 |
